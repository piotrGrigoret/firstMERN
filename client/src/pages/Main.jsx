  ...
  const addInformationHandler = async () =>{
    console.log(tempArticleObj);
      ...
      if(tempArticleObj.text !=="" && tempArticleObj.title !==""){
        const res = await axios.post("http://localhost:5000/add", {article : tempArticleObj});
    ...
    const deleteInformationHandler = async (article) =>{
      ...
      const res = await axios.delete("http://localhost:5000/delete", {data:{id:article._id}});
      console.log("res:" + res);
    };
   ...
   const onSubmitChanghes = async(event) => {
      ...
      setValues2("");
      setTempArticleObject(modelTempObj);
      await axios.post("http://localhost:5000/redax", {data:{redactObj:tempArticleObj}});
